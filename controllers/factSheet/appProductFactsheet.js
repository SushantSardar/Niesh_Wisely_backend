...
const generatePDF = (data,productName, res) => {
    ...
    const tableData = [];
    tableData.push(['Date', 'ProductName', 'Quantity', 'Buy/Sell', 'Script', 'Price', 'Target', 'StopLoss', 'Status', 'P/L']);
    ...
    filteredData.forEach(({ updatedAt,productName,quantity,dealType, script,   price1, target, stopLoss, statusValue, pnl }) => {
        const formattedDate = updatedAt.toISOString().split('T')[0];
        tableData.push([formattedDate,productName,quantity, dealType,script,   price1, target, stopLoss, statusValue, pnl]);
    ...
    content.push({text:`Total P&L: ${totalPnl}`, style: 'subheaderdata'});
    content.push({text:`Accuracy: ${accuracy.toFixed(2)}%` , style: 'subheaderdata'});
    content.push({text:`Total calls: ${totalCalls}`, style: 'subheaderdata'});
    content.push('\n');
    ...
    res.setHeader('Content-Disposition', `attachment; filename=${productName}_factsheet.pdf`);
...
const ShowFactSheetForProductInAPP = async (req, res) => {
    try {
...
module.exports = ShowFactSheetForProductInAPP;
